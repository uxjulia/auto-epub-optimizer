...
process_drop_dir() {
  ...
  while IFS= read -r -d '' filepath; do
    ...
    [ -n "$EPUB_GRAYSCALE_COVER" ] && optimizer_args+=("--grayscale-cover")
    [ -n "$EPUB_NO_EINK_QUANTIZE" ] && optimizer_args+=("--no-eink-quantize")
    [ -n "$EPUB_NO_REMOVE_FONTS" ] && optimizer_args+=("--no-remove-fonts")
    [ -n "$EPUB_NO_REMOVE_CSS" ] && optimizer_args+=("--no-remove-css")
    [ -n "$EPUB_NO_GENERATE_COVER" ] && optimizer_args+=("--no-generate-cover")
    [ -n "$EPUB_NO_CLEAN_METADATA" ] && optimizer_args+=("--no-clean-metadata")
    [ -n "$EPUB_NO_TEXT_CLEANUP" ] && optimizer_args+=("--no-text-cleanup")
    [ -n "$EPUB_NORMALIZE_QUOTES" ] && optimizer_args+=("--normalize-quotes")
    [ -n "$EPUB_KEEP_QUOTES" ] && optimizer_args+=("--keep-quotes")
    [ -n "$EPUB_NORMALIZE_DASHES" ] && optimizer_args+=("--normalize-dashes")
    [ -n "$EPUB_KEEP_DASHES" ] && optimizer_args+=("--keep-dashes")
    [ -n "$EPUB_NO_NORMALIZE_ELLIPSIS" ] && optimizer_args+=("--no-normalize-ellipsis")
    [ -n "$EPUB_SPLIT_MODE" ] && optimizer_args+=("--split" "$EPUB_SPLIT_MODE")
    [ -n "$EPUB_LIGHT_NOVEL" ] && optimizer_args+=("--light-novel")
    [ -n "$EPUB_ROTATE_RIGHT" ] && optimizer_args+=("--rotate-right")
    [ "$EPUB_SPLIT_LONG_SECTIONS" = "0" ] && optimizer_args+=("--no-split-long-sections")
    [ -n "$EPUB_SECTION_SPLIT_WORD_THRESHOLD" ] && optimizer_args+=("--section-split-word-threshold" "$EPUB_SECTION_SPLIT_WORD_THRESHOLD")
    [ -n "$EPUB_SECTION_SPLIT_BYTE_THRESHOLD" ] && optimizer_args+=("--section-split-byte-threshold" "$EPUB_SECTION_SPLIT_BYTE_THRESHOLD")
    [ -n "$EPUB_SECTION_SPLIT_HARD_BYTE_LIMIT" ] && optimizer_args+=("--section-split-hard-byte-limit" "$EPUB_SECTION_SPLIT_HARD_BYTE_LIMIT")
    ...
    [ -n "$EPUB_VERBOSE" ] && optimizer_args+=("--verbose")
