...
CROSSINK_LOCATION_MANIFEST_PATH = os.path.join('META-INF', 'x-locations.json')
CROSSINK_OPTIMIZER_MANIFEST_PATH = os.path.join('META-INF', 'crossink', 'optimizer-v1.json')
CROSSINK_LOCATION_WORDS_PER_UNIT = 64
DEFAULT_CROSSINK_REFERENCE_CHARACTERS_PER_PAGE = 1500
...
def collapse_reader_empty_spine_items(opf_path: str) -> int:
    ...
    entries = []
    for ref in _find_elements(spine, 'itemref'):
        ...
        while child_index < len(split_children):
            ...
            if natural_split_offset > 0:
                ...
                if natural_split_index > child_index:
                    ...
                    current_words = 0
                    current_bytes = fixed_bytes
                    # Begin a new chunk at the selected natural boundary rather
                    # than consuming the intervening nodes again.
...
def write_crossink_location_manifest(
    epub_dir: str,
    opf_path: str,
    characters_per_reference_page: int = DEFAULT_CROSSINK_REFERENCE_CHARACTERS_PER_PAGE,
    ...
    characters_per_reference_page = max(
        1,
        int(characters_per_reference_page or DEFAULT_CROSSINK_REFERENCE_CHARACTERS_PER_PAGE),
    )
    opf_dir = Path(opf_path).parent
    spine = []
    total_words = 0
...
def write_crossink_optimizer_manifest(epub_dir: str, opf_path: str, image_cache_entries: list[dict],
    ...
    out_path = epub_root / CROSSINK_OPTIMIZER_MANIFEST_PATH
    out_path.parent.mkdir(parents=True, exist_ok=True)
